...
async def scrape_one(img_tag):
    ...
    if img_url:
        ...
        await download_and_convert(img_url, file_name)
...
async def scrape_images(search_query):
    ...
    soup = await get_soup(unsplash_url)
    ...
    res = [await scrape_one(tag) for tag in tqdm(img_tags)]
